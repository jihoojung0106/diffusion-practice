...
import sys
sys.path.append('/root/myDiffSinger')
...
def get_pitch(wav_data, mel):
    ...
    #f0:(826,) wav_data : (212992,)
    f0 = parselmouth.Sound(wav_data, audio_sample_rate).to_pitch_ac(
        time_step=time_step / 1000, voicing_threshold=0.6,
        pitch_floor=f0_min, pitch_ceiling=f0_max).selected_array['frequency']
    #to_pitch_ac 메소드의 결과로부터 피치의 주파수 배열을 추출합니다. 이 배열은 각 시간 단계에서의 피치 주파수 값을 포함합니다.
    
    lpad = pad_size * 2
    rpad = len(mel) - len(f0) - lpad
    #mel과 f0의 사이즈를 맞추기 위한 패딩 추가 (832,)
    ...
    f0 = f0[:len(mel)]
    pitch_coarse = f0_to_coarse(f0) #Pitch를 mel scale로 맞추어 정규화 및 정수화하고, coarse하게 바꾸는 것.
...
def get_mel2ph(tg_fn, ph, mel):
    ...
    audio_sample_rate=22050
    hop_size=256
    split = [int(s *audio_sample_rate / hop_size + 0.5) for s in split]
