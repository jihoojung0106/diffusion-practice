...
class TokenTextEncoder(TextEncoder):
    ...
    def _init_vocab(self, vocab_list):
        """Initialize vocabulary with tokens from token_generator."""

        self._id_to_token = {0:PAD, 1:EOS, 2:UNK}
        non_reserved_start_index = 3
        for i, token in enumerate(vocab_list):
            if token not in RESERVED_TOKENS:
                self._id_to_token[non_reserved_start_index] = token
                non_reserved_start_index+=1
